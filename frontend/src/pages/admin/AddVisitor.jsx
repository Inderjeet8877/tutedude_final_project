...
import { ArrowLeft, UserPlus, Save, AlertCircle, Building2, Phone, Mail, User } from 'lucide-react';
...
const AddVisitor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Error occurred while issuing security clearance.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-400 tracking-tight">Authorize Pre-Entry</h1>
          <p className="text-slate-400 text-sm mt-1">Issue a new digital pass to an expected external subject.</p>
        </div>
      </div>
